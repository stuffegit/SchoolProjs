 ...
 * 2. Validates the input for syntactical errors using the valdiate_input_format
 *function.
 * 3. If the input fails validation, prompts the user to try again. If it
 *passes, proceeds.
 * 4. Calculates the expected control digit value using the luhn_algorithm
 *function.
 * 5. Compares the calculated control digit with the provided control digit and
 *outputs the result.
 *
 * @see https://sv.wikipedia.org/wiki/Personnummer_i_Sverige Reference for
 *algorithm help and logical guidance.
 * @see https://www.datorproblem.org/Personnummergenerator
 * @see https://simplycalc.com/luhn-calculate.php Tools to check program
 *accuracy.
 **/
#include <stdio.h>
#include <string.h>
...
int main(void) {
  ...
  while (!validinput) {
    ...
    validinput =
        valdiate_input_format(input, length); // check format: YYMMDD-xxxC.
  }

  int digit_character_position = 0;
  for (int i = 0; i < length; i++) { // Find and remove the dash from input to
                                     // populate the digits_only string
    if (input[i] != '-') {
      digits_only[digit_character_position++] = input[i];
    }
  }
  digits_only[digit_character_position] =
      '\0'; // Null-terminate the digits_only string
  ...
  int calculated_control_digit =
      (10 - (luhn_algorithm(digits_only, 9) % 10)) % 10;
 ...
 * It ensures the correct number of digits and that the dash is in the correct
 * position.
 *
...
int valdiate_input_format(char *input, int length) {
  ...
  if (length != 11 || sscanf(input, "%6[0-9]-%4[0-9]", check_first_part,
                             check_second_part) != 2) {
 ...
 *
 * This function adjusts the year based on the YEAR_THRESHOLD to handle
 * two-digit years and verifies the day of the month, respecting leap years for
 * February.
 *
...
int validate_date(int year, int month, int day) {
  ...
  int days_in_month[] = {
      31, 28, 31, 30, 31, 30,
      31, 31, 30, 31, 30, 31}; // Check day validity based on month
 ...
 *
 * @param input The digit string (without dashes) for which to calculate the
 * checksum.
 * @param length The length of the input string.
...
int luhn_algorithm(char *input, int length) {
  ...
  for (int i = 0; i < length; i++) { // Loop through each char from the input
    ...
    if (alternate) { // multiply by 2 or 1, alternating with the alternate
                     // variable
      double_digit = digit * 2;
      if (double_digit > 9) { // If doubled result is > 9 making it two numbers
                              // we sum the digits
